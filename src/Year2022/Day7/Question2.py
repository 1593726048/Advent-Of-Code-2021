...
def free_up(folder_dict, size_to_free):
    able_to_free = []
    for key in folder_dict:
        if isinstance(folder_dict[key], dict):
            if count_folder_size(folder_dict[key]) >= size_to_free:
                able_to_free.append(count_folder_size(folder_dict[key]))
                able_to_free.extend(free_up(folder_dict[key], size_to_free))
    return able_to_free
        ...
        while i < len(lines):
            if lines[i][0] == "$":
                if lines[i][2:4] == "ls":
                    ...
                    pass
            i += 1
    print(min(free_up(folder_dict, count_folder_size(folder_dict) - (70000000 - 30000000))))
